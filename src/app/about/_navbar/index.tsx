...
const LuxuryNavbar = () => {
  ...
  return (
    <header className="w-full bg-none">
          ...
          <div className="flex items-center justify-between px-4 md:px-8">
            ...
            <div className="flex items-center">
              ...
              >
                ...
                <Menu 
                  size={24} 
                  className={`${isScrolled ? "text-white" : "text-white"} group-hover:text-[#9f3417] transition-colors duration-300`}
                />
              </button>
            </div>
            ...
            <Link href="/" className="relative flex items-center z-20">
              ...
              <img src={`${isScrolled ? "/logo.webp" : "/logo.webp"}`} alt="Logo" className="h-14 inline-block py-2" />
              </div>
            </Link>
            ...
            <div className="flex items-center gap-4 z-50">
              ...
              <div className={`border ${isScrolled ? "text-white" : "border-white"}  rounded-md px-3 py-1 backdrop-blur-sm bg-white/5 hover:bg-white/20 transition-colors duration-300`}>
                <Link href="/en" className={`${isScrolled ? "" : "text-white"} text-xs uppercase tracking-wider font-light hover:text-[#9f3417] transition-colors`}>EN</Link>
                <span className={`${isScrolled ? "" : "text-white"}  mx-2`}>|</span>
                <Link href="/ar" className={`${isScrolled ? "]" : "text-white"} text-xs uppercase tracking-wider font-light hover:text-[#9f3417] transition-colors`}>AR</Link>
              </div>
            </div>
          </div>
        </div>
      ...
      <motion.div
        className="fixed top-0 left-0 right-0 bottom-0 bg-gradient-to-b from-[#F8F8F8]/98 to-[#F0F0F0]/95 backdrop-blur-xl z-50"
        initial={{ x: "-100%" }}
      ...
      >
        ...
        >
          ...
          <X size={24} className="text-[#9f3417] group-hover:text-[#9f3417] transition-colors duration-300" />
        </button>

        {/* Menu Brand Logo - Top Center */}
        <div className="absolute top-8 left-1/2 transform -translate-x-1/2 z-40">
          <div className="flex items-center">
            <div className="h-[1px] w-8 bg-[#9f3417] rounded-full opacity-70"></div>
            <div className="mx-3 text-center">
              <div className="text-[#9f3417] text-base font-normal tracking-[0.2em] uppercase">RELAAM</div>
              <div className="text-[#555555] text-[9px] tracking-[0.15em] uppercase mt-0.5 font-medium">Developments</div>
            </div>
            <div className="h-[1px] w-8 bg-[#9f3417] rounded-full opacity-70"></div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 mx-auto max-w-6xl">
            {menuItems.map((item, index) => (
              ...
              >
                ...
                >
                  {/* Item number */}
                  <p className="text-[#9f3417] text-xl font-medium">0{item.id}</p>
                  ...
                  <div className="flex flex-col items-start gap-1">
                    <div className="flex items-center">
                      ...
                      {item.hasSubmenu && (
                        <ChevronDown 
                          className={`ml-4 text-[#9f3417] transition-transform duration-300 ${expandedSubmenu === item.id ? 'rotate-180' : ''}`}
                          size={24}
                        />
                      )}
                    </div>
                      ...
                      {item.submenu?.map((subItem) => (
                        <li key={subItem.id} className="group">
                          <Link 
                            ...
                            onClick={() => setShowMenu(false)}
                          >
                            <ChevronRight className="mr-2 text-[#9f3417]/60 group-hover:text-[#9f3417] transition-colors" size={16} />
                            {subItem.title}
                          </Link>
                        </li>
                      ))}
        ...
        >
          <div className="max-w-4xl mx-auto h-full">
            <div className="relative h-full w-full">
              <img
                src={selectedMenu ? `/images/community/${selectedMenu.id}.jpg` : "/images/community/1.jpg"}
        ...
        <div className="absolute bottom-12 left-0 right-0 z-50">
          <div className="flex items-center justify-center gap-10 text-[#555555] font-light text-sm uppercase tracking-widest">
            {secondaryMenu.map((item) => (
              <Link key={item.id} href={item.href} className="group" onClick={() => setShowMenu(false)}>
                <div className="flex flex-col items-center gap-1 cursor-pointer">
                  <span className="group-hover:text-[#9f3417] transition-colors duration-300">{item.title}</span>
                  <div className="w-0 h-0.5 bg-[#9f3417]/80 group-hover:w-full transition-all duration-300"></div>
                </div>
              </Link>
            ))}
